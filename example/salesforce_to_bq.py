...
Tested with the newest version of airflow located on GitHub:
    pip install git+https://github.com/apache/incubator-airflow.git

And then to make sure GCP dependencies are installed:
    pip install airflow[gcp_api]

ImportError: dlopen(python3.4/site-packages/lxml/etree.so, 2): Library not loaded: libxml2.2.dylib
  Referenced from: lib/python3.4/site-packages/lxml/etree.so
...
FIX:
    ...
    http://stackoverflow.com/a/31607751/6288435

You have to create bucket in GCS to store files before you can upload files there.  
